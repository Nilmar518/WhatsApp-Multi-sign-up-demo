...
function ReservationCard({ r, onCancel, cancellingId }: { r: Reservation; onCancel?: (pmsBookingId: string) => void; cancellingId?: string | null }) {
  ...
  return (
    <div className="rounded-xl border border-edge bg-surface-raised px-4 py-3 shadow-sm transition-shadow hover:shadow-md">
      ...
      {/* Cancel button — only for manual bookings that aren't already cancelled */}
...
const CHANNEL_BADGE_STYLES: Record<string, string> = {
  ...
  vrbo: 'bg-surface-subtle text-content-2',
  walkin: 'bg-surface-subtle text-content-2',
  maintenance: 'bg-caution-bg text-caution-text',
  owner_stay: 'bg-brand-subtle text-brand',
  direct: 'bg-ok-bg text-ok-text',
};
